...
export default function MayoristasPage() {
  ...
  useEffect(() => {
    fetch("/admin/mayoristas", { credentials: "include" })
      .then((r) => r.json())
      .then((d) => { setMayoristas(d.mayoristas || []); setLoading(false) })
  }, [])
  ...
  const cambiarEstado = (id: string, estado: string) => {
    ...
    fetch(`/admin/mayoristas/${id}`, {
      ...
      .then(() => fetch("/admin/mayoristas", { credentials: "include" }))
      .then((r) => r.json())
      .then((d) => { setMayoristas(d.mayoristas || []); setUpdating(null) })
  }
          ...
          <table className="w-full text-sm border-collapse">
            <thead>
              <tr className="border-b border-gray-200">
                <th className="text-left pb-3 font-medium text-gray-600">Empresa</th>
                <th className="text-left pb-3 font-medium text-gray-600">CUIT</th>
                <th className="text-left pb-3 font-medium text-gray-600">Email</th>
                <th className="text-left pb-3 font-medium text-gray-600">Ubicación</th>
                <th className="text-left pb-3 font-medium text-gray-600">Estado</th>
                <th className="text-left pb-3 font-medium text-gray-600">Acciones</th>
              </tr>
            </thead>
            <tbody>
              {mayoristas.map((m) => (
