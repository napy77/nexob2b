-- Pasa a "aprobado" los productos que matchearon con DIA (categoría/descripción/foto/marca
...
-- rechazó a mano, este script no lo reabre).
--
...
CREATE TEMP TABLE staging_dia_match_ean (
  ean text,
  pasillo text,
  rubro text,
  subrubro text,
...
\copy staging_dia_match_ean (ean, pasillo, rubro, subrubro, descripcion, marca, imagen_url, nombre_dia) FROM 'data.csv' WITH (FORMAT csv, HEADER true, ENCODING 'UTF8')

UPDATE producto_maestro pm
SET estado = 'aprobado',
    updated_at = now()
FROM staging_dia_match_ean stg
...
JOIN staging_dia_match_ean stg ON stg.ean = pm.ean
