...
export const PUT = async (req: MedusaRequest, res: MedusaResponse) => {
  ...
  const { rows: [row] } = await pool.query(
    `UPDATE nexo_api_key SET ${sets.join(", ")} WHERE id = $${i} AND deleted_at IS NULL RETURNING id, nombre, tipo, activa, webhook_url`,
    vals
  )
...
// DELETE /admin/nexo-api-keys/:id
export const DELETE = async (req: MedusaRequest, res: MedusaResponse) => {
  const pool = getPool()
  const { rowCount } = await pool.query(
    `UPDATE nexo_api_key SET deleted_at = now(), activa = false WHERE id = $1 AND deleted_at IS NULL`,
    [req.params.id]
  )
